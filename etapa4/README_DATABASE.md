...
1. **Registros Imutuáveis (Row Level Security):** A tabela de logs de auditoria foi configurada sob a regra de *append-only*. Através das políticas de RLS, comandos de `UPDATE` e `DELETE` são bloqueados nativamente pelo Supabase, garantindo conformidade.
...
![Diagrama do Banco de Dados](database.jpeg)
